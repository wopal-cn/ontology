...
import { Finding, CheckResult, ScanResult, IOCData } from "./types.js";
import {
  RISK_SCORE_WEIGHTS,
  HIGH_RISK_THRESHOLD,
  MAX_RISK_SCORE,
...
export async function scanSkill(
  ...
  for (const metadata of CHECK_METADATA) {
    ...
    try {
      ...
      logger.debug(
        `检查完成: ${metadata.name} - ${checks[metadata.id].status}`,
      );
    } catch (error) {
      logger.error(`检查失败: ${metadata.name}`, {
...
function calculateSummary(checks: Record<string, CheckResult>): {
  critical: number;
  warning: number;
  passed: number;
} {
