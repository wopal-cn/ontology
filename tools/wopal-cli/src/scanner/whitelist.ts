...
export function filterWhitelistedFindings(findings: Finding[], whitelist: string[]): Finding[] {
  const filtered = findings.filter(finding => !isWhitelisted(finding.pattern, whitelist));
  
  if (findings.length !== filtered.length) {
    logger.debug(`白名单过滤: ${findings.length} -> ${filtered.length} (移除 ${findings.length - filtered.length} 个误报)`);
  }
