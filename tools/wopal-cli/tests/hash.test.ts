...
import os from 'os';
import { computeSkillFolderHash } from '../src/utils/hash.js';
...
describe('computeSkillFolderHash', () => {
  ...
  it('should return different hash for different content', async () => {
    await fs.writeJson(path.join(tempDir, 'SKILL.md'), { name: 'test-skill-v1' });
    const hash1 = await computeSkillFolderHash(tempDir);

    await fs.writeJson(path.join(tempDir, 'SKILL.md'), { name: 'test-skill-v2' });
    const hash2 = await computeSkillFolderHash(tempDir);
  ...
  it('should exclude node_modules directory', async () => {
    ...
    await fs.writeJson(path.join(tempDir, 'node_modules', 'package.json'), { name: 'dep' });
  ...
  it('should handle nested directories', async () => {
    ...
    await fs.writeJson(path.join(tempDir, 'scripts', 'test.js'), { script: true });
  ...
  it('should throw error for non-existent path', async () => {
    await expect(computeSkillFolderHash('/non/existent/path')).rejects.toThrow();
  });
});
