...
describe('Install Command Integration Tests', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'wopal-cli-integration-'));
    ...
    process.env.SKILL_INBOX_DIR = inboxDir;
  });

  afterEach(async () => {
    await fs.remove(tempDir);
    delete process.env.SKILL_INBOX_DIR;
  });

  async function createTestSkill(skillDir: string, name: string): Promise<void> {
  ...
  it('should install skill from INBOX to project-level', async () => {
    ...
    const output = execSync(
      `node ${CLI_PATH} skills install ${skillName}`,
  ...
  it('should handle --force flag', async () => {
    ...
    execSync(
      `node ${CLI_PATH} skills install ${skillName} --skip-scan`,
      { cwd: projectDir, encoding: 'utf-8' }
    );
  ...
  it('should reject symlink mode', async () => {
    ...
    try {
      execSync(
        `node ${CLI_PATH} skills install ${skillName} --mode symlink`,
        { cwd: projectDir, encoding: 'utf-8' }
      );
