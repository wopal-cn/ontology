...
The scenario "Uppercase frontmatter keys" claiming case-insensitive key matching is removed. The implementation uses case-sensitive key access (`parsed.globs`, `parsed.keywords`, `parsed.tools`). Frontmatter keys MUST be lowercase.
