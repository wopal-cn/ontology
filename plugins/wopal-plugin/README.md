...
| `wopal_task_output` | 查询任务状态、进度、完整历史；`action="complete"` 确认完成 |
| `wopal_task_cancel` | 取消运行中的任务 |
| `wopal_task_reply` | 向等待中的任务发送消息恢复执行；`interrupt=true` 中断纠偏 |
| `wopal_task_diff` | 查看任务产生的文件变更摘要 |
...
```
wopal_task_output(task_id="wopal-task-xxx")
```
...
running → session.idle → diagnoseIdle() → {completed | waiting | error}
         ↑                                           │
         │                                           ↓
         └─────── wopal_task_reply ←── [WOPAL TASK WAITING]
```
